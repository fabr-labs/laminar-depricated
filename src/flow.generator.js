...
export function* flowGeneratorFn({ flow, middleware, args, goto }){
  for (let [index, directive] of goto ? gotoStep(goto, flow(args).entries()) : flow(args).entries()) {    
    try {
      if (directive.calls) {
        yield meta => Promise.all(directive.calls.map(asyncDirective => applyMiddleware(callFn, middleware)({ directive: asyncDirective, meta })));
      } else {
        yield meta => applyMiddleware(callFn, middleware)({ directive, meta });
      }
    } catch (error) {
      console.warn(error);
      return;
